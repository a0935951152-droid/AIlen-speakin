...
啟動/測試/通道的操作指令全在 **《指令.md》**（runbook）。
環境前提：venv 在本機 NVMe（`/home/a0935951152/speakin-venv`），repo 內 `.venv`
是指向它的 symlink（venv 無 pip，裝套件用 `uv pip install --python .venv/bin/python`）。
