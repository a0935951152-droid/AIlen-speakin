...
bash deploy/dev/gateway.sh             # gateway :8800（LiveKit token + NATS→WS 事件橋）

# 前端（frontend/ 是 npm workspace；node 18+）
cd frontend && npm install             # 首次
npm run dev                            # viewer 開發伺服器 :5173（--host，LAN 裝置可連）
npm run typecheck && npm run build     # CI 同款檢查
npm run gen:types                      # schema 變更後重生 packages/sdk/src/events.gen.ts

# 對外 https（麥克風需要安全來源）：vite 已同源代理 /api /ws（gateway）與 /lk（LiveKit 信令）
cloudflared tunnel --url http://localhost:5173   # 免帳號 quick tunnel，回一個 trycloudflare.com URL
# 注意：tunnel 只載 https/WS（頁面、字幕、信令）；WebRTC 媒體流走 UDP 直連——
# 同網段裝置自動走區網 host candidate；真外網裝置要聽到聲音需 TURN（LiveKit Cloud / 自架）
